...
app.get("/Bio-Barat-Biobolt", (req, res, next) => {
  const pageTitle = "Bio-Barát Biobolt";
  const oldalData = OldalAdatokData.find(
    (oldal) => oldal.pageTitle === pageTitle
  );
  if (oldalData) {
    res.render("bio", { pageTitle: oldalData.pageTitle, data: oldalData.data });
  } else {
...
app.get("/Bio-Barat-Biobolt", (req, res, next) => {
  res.render("bio", {
    pageTitle: "Bio-Barát Biobolt",
    pageData: OldalAdatokData,
  });
});
*/
